...
def validate_input_security(
    value: str, field_name: str, max_length: int = MAX_INPUT_LENGTH
) -> str:
    """
    ...
    try:
        if value.strip().startswith(("{", "[")):
            ...
            if isinstance(parsed, dict) and (
                "__proto__" in str(parsed) or "constructor" in str(parsed)
            ):
                raise SecurityValidationError(
                    f"{field_name} contains prototype pollution attempt"
                )
        ...
        raise SecurityValidationError(
            f"{field_name} contains malicious nested structure"
        )
