...
from .validation import (
    ...
    validate_input_security as base_validate
)
...
ENHANCED_COMPILED = [re.compile(p, re.IGNORECASE | re.MULTILINE) for p in ENHANCED_PATTERNS]
ALL_PATTERNS = COMPILED_PATTERNS + ENHANCED_COMPILED

# Attack severity classification for threat intelligence
SEVERITY_CRITICAL = {
    "container_escape", "privilege_escalation", "code_execution", 
    "system_access", "credential_theft"
}

SEVERITY_HIGH = {
    "command_injection", "prompt_injection", "deserialization_attack",
    "supply_chain_attack", "ssrf", "metadata_access"
}

SEVERITY_MEDIUM = {
    "sql_injection", "xss", "template_injection", "path_traversal",
    "nosql_injection", "jwt_manipulation"
}

SEVERITY_LOW = {
    "information_disclosure", "validation_bypass", "generic_injection"
}
...
def validate_enhanced_security(
    ...
    metadata = {
        ...
        "advanced_detections": len([t for t in threats_detected if t in ["obfuscated_payload", "pattern_bombing", "encoding_obfuscation", "length_attack"]])
    }
