...
class MCPDiscoveryAdapter:
    ...
    def __init__(self, mcp_config: Optional[MCPConfig] = None):
        ...
        self._discovery_cache: Optional[MCPDiscoveryResponse] = None
    ...
    async def _discover_servers(self, request: MCPDiscoveryRequest) -> List[MCPServerInfo]:
        """
        ...
        for server_name, server_config in self._mcp_config.servers.items():
            # Apply server filter
            if request.server_filter and not fnmatch.fnmatch(server_name, request.server_filter):
                continue
        ...
        for server_info in servers:
            ...
            try:
                ...
                for tool in server_tools:
                    ...
                    # Apply tool filter
                    if request.tool_filter and not fnmatch.fnmatch(tool_name, request.tool_filter):
                        continue
    ...
    def _apply_filters(
        self, 
        cached_response: MCPDiscoveryResponse, 
        request: MCPDiscoveryRequest
    ) -> MCPDiscoveryResponse:
        """
        ...
        for server in cached_response.servers:
            if request.server_filter and not fnmatch.fnmatch(server.name, request.server_filter):
                continue
        ...
        for tool in cached_response.tools:
            if request.server_filter and not fnmatch.fnmatch(tool.server_name, request.server_filter):
                continue
                
            if request.tool_filter and not fnmatch.fnmatch(tool.tool_name, request.tool_filter):
                continue
