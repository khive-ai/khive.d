...
class CapabilityContext(BaseModel):
    ...
    principal_id: str = Field(
        ..., description="ID of the principal (agent/user) making the request"
    )
    capability_token: str | None = Field(
        None, description="JWT capability token for authorization"
    )
    request_id: str | None = Field(
        None, description="Unique request identifier for audit trail"
    )
    source_ip: str | None = Field(None, description="Source IP address if applicable")
...
class MCPToolRequest(BaseModel):
    ...
    timeout: int | None = Field(
        None, description="Timeout in seconds for tool execution"
    )
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
...
class MCPDiscoveryRequest(BaseModel):
    ...
    server_filter: str | None = Field(
        None, description="Filter servers by name pattern"
    )
    tool_filter: str | None = Field(None, description="Filter tools by name pattern")
    include_disabled: bool = Field(
        False, description="Include disabled servers in discovery"
    )
    include_schemas: bool = Field(
        True, description="Include parameter schemas in tool info"
    )
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
...
class MCPServerInfo(BaseModel):
    ...
    server_info: dict[str, Any] | None = Field(
        None, description="Server-provided information"
    )
...
class MCPDiscoveryResponse(BaseModel):
    ...
    servers: list[MCPServerInfo] = Field(
        default_factory=list, description="Discovered servers"
    )
    tools: list[MCPToolInfo] = Field(
        default_factory=list, description="Discovered tools"
    )
    ...
    discovery_time_ms: float | None = Field(
        None, description="Time taken for discovery in milliseconds"
    )
...
class MCPServerAdapterConfig(BaseModel):
    ...
    capability_context: CapabilityContext | None = Field(
        None, description="Security capability context"
    )
    connection_timeout: int = Field(30, description="Connection timeout in seconds")
    call_timeout: int = Field(60, description="Tool call timeout in seconds")
    enable_audit_logging: bool = Field(
        True, description="Enable audit logging for tool calls"
    )
