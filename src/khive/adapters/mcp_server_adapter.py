...
class MCPServerAdapter:
    ...
    def __init__(self, config: MCPServerAdapterConfig, mcp_config: Optional[MCPConfig] = None):
        """
    ...
    async def from_obj(self, data: MCPToolRequest, **kwargs) -> MCPToolResponse:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                client.call_tool(data.tool_name, data.arguments),
                timeout=timeout
            )
    ...
    async def _get_client(self) -> MCPClient:
        ...
        if client_key not in _adapter_clients:
            ...
            if self.config.server_name not in self._mcp_config.servers:
                raise ValueError(f"Server '{self.config.server_name}' not found in MCP configuration")
            ...
            if await client.connect():
                ...
                raise RuntimeError(f"Failed to connect to MCP server '{self.config.server_name}'")
    ...
    async def _validate_capability(self, request: MCPToolRequest) -> None:
        ...
        print(f"CAPABILITY_CHECK: {self.config.capability_context.principal_id} -> "
    ...
    async def _audit_log_tool_call(
        ...
        audit_data = {
            ...
            "principal_id": (
                ...
                if self.config.capability_context else "unknown"
            ),
            ...
            "request_id": (
                ...
                if self.config.capability_context else None
            ),
        }
