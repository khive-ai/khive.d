...
from .base import CLIResult, GitBasedCLICommand
from .utils import (
    BaseConfig,
    git_run,
    info_msg,
...
class CommitCommand(GitBasedCLICommand):
    ...
    def _validate_args(self, args: argparse.Namespace) -> None:
        ...
        # Validate message strategy
        if (
            not args.interactive
            and not args.message
            and not (args.type and args.subject)
        ):
            raise ValueError(
                "No commit message strategy: Provide a positional message, "
                "or --type and --subject, or use --interactive."
            )
        ...
        try:
            ...
            if not args.amend:
                ...
                if not staged_something:
                    if config.allow_empty_commits and args.allow_empty:
                        info_msg(
            ...
            commit_message = self._build_commit_message(args, config)
            ...
            # Perform the commit
            commit_result = self._perform_commit(commit_message, args, config)
            ...
            # Handle push
            push_result = self._handle_push(args, config)
    ...
    def _build_commit_message(
        self, args: argparse.Namespace, config: CommitConfig
    ) -> str | None:
        ...
        return self._build_structured_message(args)

    def _build_structured_message(self, args: argparse.Namespace) -> str | None:
        ...
        extra_info = []
        if args.search_id:
            extra_info.append(f"(search: {args.search_id})")
        if args.closes:
        ...
        if args.by:
            if body_parts or "\n\n" not in full_message:
                full_message += "\n\n"
            full_message += f"Committed-by: {args.by}"
    ...
    def _perform_commit(
        self, commit_message: str, args: argparse.Namespace, config: CommitConfig
    ) -> dict[str, Any]:
        ...
        if args.allow_empty and not args.amend:
    ...
    def _handle_push(
        ...
        should_push = args.push if args.push is not None else config.default_push
...
# --- CLI Entry Point ---
def main() -> None:
    """Main entry point for the khive commit command."""
