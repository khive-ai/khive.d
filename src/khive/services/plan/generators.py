...
from .models import (
    DecompositionCandidate, StrategyCandidate, PlannerRequest, 
    TaskPhase, AgentRecommendation, CoordinationStrategy, QualityGate,
    ComplexityLevel
)
...
class SelfConsistencyEngine:
    ...
    def extract_consistent_structure(
        ...
        for candidate in candidates:
            ...
            if complexity_counter[candidate.estimated_complexity] >= len(candidates) * threshold:
                score += 1
...
Focus on practical, implementable phases that build toward the final goal."""
    ...
    async def generate_candidates(
        ...
        generators = [g for g in self.config.generators if g.get("role") == "decomposer"]
        ...
        candidates = [
            ...
            if isinstance(result, DecompositionCandidate)
        ]
...
Return structured JSON with reasoning and complete TaskPhase objects."""
    ...
    async def generate_strategies(
        ...
        generators = [g for g in self.config.generators if g.get("role") == "strategist"]
        
        for i, decomposition in enumerate(decompositions[:target_count]):
            if i < len(generators):
                generator = generators[i % len(generators)]
                for temp in generator.get("temps", [0.4])[:1]:  # Use one temp per decomposition
                    task = asyncio.create_task(
    ...
    async def refine_and_validate(
        ...
        async with self.semaphore:
            try:
                ...
                generator = generators[0] if generators else {"provider": "openrouter", "model": "google/gemini-2.0-flash-001"}
                ...
                instruction = self.REFINER_PROMPT.format(
                    candidates_json=json.dumps([
                        {"phases": [phase.model_dump() for phase in candidate.phases]}
                ...
                return top_candidates[0] if top_candidates else self._create_emergency_fallback(request)
    
    def _apply_validation_gates(self, candidate: StrategyCandidate) -> StrategyCandidate:
        ...
        if len(candidate.phases) < 3:
            ...
            while len(candidate.phases) < 3:
                candidate.phases.append(TaskPhase(
        ...
        for phase in candidate.phases:
            if not phase.agents:
                phase.agents.append(AgentRecommendation(
                    role="implementer",
                    domain="software-architecture", 
                    priority=0.7,
