...
def fetch_github_issue(issue_num: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"❌ Error fetching issue #{issue_num}: {result.stderr}", file=sys.stderr)
            return None
...
Examples:
  ...
  khive plan "Add caching layer" --time-budget 120 --json
    ...
    parser.add_argument(
        "task_description",
        nargs="?",
        help="Description of the task to plan"
    )
    
    parser.add_argument(
        "--issue", 
        type=str,
        help="GitHub issue number to use as context"
    )
...
def main() -> None:
    ...
    if not args.task_description and not args.issue:
        print("❌ Error: Either task description or --issue must be provided", file=sys.stderr)
        parser.print_help()
        sys.exit(1)
    ...
    if args.issue:
        ...
        if issue_data:
            ...
            if not task_description:
                task_description = issue_data.get('title', f"Work on issue #{args.issue}")
        else:
            print(f"⚠️  Could not fetch issue #{args.issue}, proceeding without issue context")
    ...
    try:
        # Run planning
        response = asyncio.run(run_planning(
            task_description=task_description,
            context=context,
