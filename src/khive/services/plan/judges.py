...
from .models import JudgeScore, PairwiseComparison, StrategyCandidate
...
Consider the trade-offs carefully and justify your decision."""
    ...
    async def score_candidates(
        ...
        for candidate_idx in range(len(candidates)):
            for judge_idx in range(len(self.judges_config)):
                if task_idx < len(results) and isinstance(
                    results[task_idx], JudgeScore
                ):
    ...
    async def pairwise_comparisons(
        ...
        comparisons = [
            result for result in results if isinstance(result, PairwiseComparison)
        ]
        ...
        async with self.semaphore:
            try:
                ...
                plan_json = json.dumps(
                    [phase.model_dump() for phase in candidate.phases], indent=2
                )
        ...
        async with self.semaphore:
            try:
                ...
                plan_a_json = json.dumps(
                    [phase.model_dump() for phase in candidate_a.phases], indent=2
                )
                plan_b_json = json.dumps(
                    [phase.model_dump() for phase in candidate_b.phases], indent=2
                )
