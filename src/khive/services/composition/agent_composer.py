...
class AgentComposer:
    ...
    def generate_prompt(
        ...
        # COORDINATION FIRST - Override domain-specific thinking
        if include_coordination:
            coordination_id = agent_spec.get("coordination_id")
            if coordination_id:
                prompt_parts.append("🚨 COORDINATION FIRST - CRITICAL DIRECTIVES 🚨")
                prompt_parts.append("YOU ARE PART OF A COORDINATED EFFORT TO INTEGRATE EXISTING SYSTEMS")
                prompt_parts.append("Your role+domain expertise serves the BIGGER COORDINATION TASK, not individual excellence")
                prompt_parts.append("")
                prompt_parts.append("BEFORE doing ANYTHING:")
                prompt_parts.append("1. ANALYZE EXISTING ARCHITECTURE: What files/systems already exist?")
                prompt_parts.append("2. INTEGRATION FOCUS: How do existing systems connect? What needs simple modification?")
                prompt_parts.append("3. NO NEW SYSTEMS: Modify existing code, don't create parallel architectures")
                prompt_parts.append("4. COORDINATION AWARENESS: What are other agents doing? How do you fit in?")
                prompt_parts.append("")
                prompt_parts.append("❌ EXPLICITLY FORBIDDEN:")
                prompt_parts.append("❌ Creating new core modules when existing ones can be modified")
                prompt_parts.append("❌ Over-engineering solutions (quantum/evolutionary/performance optimization)")
                prompt_parts.append("❌ Working in isolation without considering other agents' work")
                prompt_parts.append("❌ Building parallel systems instead of integrating existing ones")
                prompt_parts.append("❌ Creating >100 lines of code for 'integration' tasks")
                prompt_parts.append("")
                prompt_parts.append("✅ YOUR SUCCESS METRICS:")
                prompt_parts.append("✅ Modified existing systems to work together")
                prompt_parts.append("✅ Built on other agents' work effectively")
                prompt_parts.append("✅ Simple, practical solutions that actually work")
                prompt_parts.append("✅ Frontend connects to backend, users can accomplish tasks")
                prompt_parts.append("")
                prompt_parts.append("🔍 PEER VALIDATION PROTOCOL - TRUST BUT VERIFY")
                prompt_parts.append("NEVER take other agents' deliverables at face value:")
                prompt_parts.append("")
                prompt_parts.append("1. **BEFORE using other agents' work**:")
                prompt_parts.append("   - Read their deliverable documents")
                prompt_parts.append("   - Check actual files they claim to have created/modified")
                prompt_parts.append("   - Test their claimed functionality independently")
                prompt_parts.append("   - Verify integration points actually work")
                prompt_parts.append("")
                prompt_parts.append("2. **VALIDATE CLAIMED ARTIFACTS**:")
                prompt_parts.append("   - If agent claims 'implemented API endpoints' → Test the endpoints")
                prompt_parts.append("   - If agent claims 'fixed integration' → Verify the integration works")
                prompt_parts.append("   - If agent claims 'created components' → Check components exist and function")
                prompt_parts.append("   - If agent claims 'completed coordination' → Test coordination system")
                prompt_parts.append("")
                prompt_parts.append("3. **REPORT DISCREPANCIES**:")
                prompt_parts.append("   - If claimed work doesn't exist: Flag in your deliverable")
                prompt_parts.append("   - If functionality is broken: Document the issues") 
                prompt_parts.append("   - If over-engineering detected: Report quantum/evolutionary code")
                prompt_parts.append("   - If integration fails: Provide specific error details")
                prompt_parts.append("")
                prompt_parts.append("4. **BUILD ONLY ON VERIFIED FOUNDATIONS**:")
                prompt_parts.append("   - Don't assume other agents' work is correct")
                prompt_parts.append("   - Test dependencies before building on them")
                prompt_parts.append("   - Fix broken foundations rather than working around them")
                prompt_parts.append("")
                prompt_parts.append("⚠️ REMEMBER: Other agents can hallucinate, lie, or misunderstand. Verify everything.")
                prompt_parts.append("")
