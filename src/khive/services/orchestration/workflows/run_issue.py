...
async def run_issue(issue_plan: IssuePlan, **kw) -> bool:
    ...
    issue = await Issue.get(issue_number, issue_plan)
    if issue.content.operation_status == "completed":
        logger.info(
            f"🔵 Skipping already completed issue #{issue.content.issue_number}"
        )
        return True

    _current_timestamp = datetime.now().isoformat()
    print(f"\n🔄 Running Issue #{issue_number} at {_current_timestamp}")
    ...
    while not gate_passed and num_attempts > 0:
        logging.info(
            f"🔄 Attempting issue #{issue.content.issue_number} (remaining attempts: {num_attempts})"
        )
        _a = b.add_operation(
            "execute_issue",
            issue=issue,
    ...
    logger.info(
        f"✅ Issue #{issue.content.issue_number} completed successfully with git cycle"
    )
