...
class OrchestrationSession:
        ...
        try:
            if self.daemon_client.is_running():
                coordination_info = self.daemon_client.coordinate_start(
                    f"Orchestration session: {self.flow_name}", self.session_id
                )
    ...
    async def execute_fanout(
        self, config: FanoutConfig, max_agents: int = 8, visualize: bool = False
    ):
        ...
        try:
            ...
            if self.daemon_client.is_running():
                self.daemon_client.coordinate_complete(
                    self.session_id, self.session_id, json.dumps(self.results), []
                )
        ...
        **kwargs,
        ...
        try:
            ...
            await session.initialize(
                system=plan.system, resume_from=resume_from, **init_kwargs
            )
            ...
            if plan.pattern == FanoutPatterns.FANOUT:
                ...
                if not plan.refinement_config:
                    raise ValueError(
                        "Refinement config required for gated refinement pattern"
                    )
            ...
            execution = IssueExecution(success=True, result=result)
...
async def orchestrate_task(
    ...
    **kwargs,
    ...
    return await engine.orchestrate_from_plan(
        plan=plan, max_agents=max_agents, visualize=visualize, **kwargs
    )
...
async def orchestrate_issue(issue_num: str | int, **kwargs) -> dict[str, Any]:
    ...
    if not issue:
        raise ValueError(
            f"Issue #{issue_num} not found. Create a plan first with 'khive plan'"
        )

    return await engine.orchestrate_from_plan(plan=issue.content.issue_plan, **kwargs)
...
def _convert_to_issue_plan(
    ...
    fanout_config = FanoutConfig(
        ...
        planning_instruction=plan_info.get(
            "planning_instruction", f"Plan: {task_description}"
        ),
...
def main():
    ...
    task_parser.add_argument(
        "--pattern",
        default="fanout",
        choices=["fanout", "fanout_with_gated_refinement"],
    )
    task_parser.add_argument(
        "--max-agents", type=int, default=8, help="Maximum number of agents"
    )
    task_parser.add_argument(
        "--visualize", action="store_true", help="Visualize workflow"
    )
    ...
    sessions_parser = subparsers.add_parser(
        "sessions", help="Manage orchestration sessions"
    )
