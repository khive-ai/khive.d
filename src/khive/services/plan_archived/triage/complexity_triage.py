...
class TriageConsensus(BaseModel):
    ...
    complexity_votes: dict[str, int] = Field(
        default_factory=lambda: {"simple": 1, "complex": 0}
    )
...
class ComplexityTriageService:
    ...
    def _build_consensus(self, votes) -> "TriageConsensus":
        ...
        if hasattr(votes[0], "complexity"):
            ...
            avg_confidence = sum(getattr(v, "confidence", 0.8) for v in votes) / len(
                votes
            )
    ...
    async def _evaluate_with_llm(self, task_description: str) -> "TriageConsensus":
        ...
        enhanced_consensus.reasoning = (
            consensus.consensus_reasoning or "Triage assessment"
        )
