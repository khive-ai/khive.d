...
class CoordinationRegistry:
    ...
    def register_agent_work(self, agent_id: str, task: str, files: List[str] = None) -> Dict[str, Any]:
        ...
        for existing_agent, work in self.active_agents.items():
            if existing_agent != agent_id:
                ...
                if task.lower() in work.task.lower() or work.task.lower() in task.lower():
        ...
        if file_path in self.file_locks:
            ...
            if lock.agent_id != agent_id:
                ...
                return {
                    ...
                    "expires_in_seconds": max(0, lock.lock_duration_seconds - (time.time() - lock.locked_at)),
                    "suggestion": "Wait for lock to expire or work on different file"
                }
        
        # Grant the lock
        self.file_locks[file_path] = FileEdit(
            file_path=file_path,
            agent_id=agent_id
        )
    ...
    def release_file_lock(self, agent_id: str, file_path: str) -> Dict[str, Any]:
        ...
        if file_path in self.file_locks:
            if self.file_locks[file_path].agent_id == agent_id:
    ...
    def share_artifact(self, agent_id: str, content: str, file_path: Optional[str] = None) -> str:
        """
            ...
            "locked_files": [
                {
                    "file": lock.file_path,
                    "locked_by": lock.agent_id,
                    "expires_in": max(0, lock.lock_duration_seconds - (time.time() - lock.locked_at))
                }
