    ...
    try:
        ...
        if not agent_id and session_id:
            ...
            if unmapped_agents:
                ...
                print(
                    f"🔗 Auto-mapped session {session_id[:8]}... to {agent_id}",
                    file=sys.stderr,
                )
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                if response.status_code == 409:  # Conflict - file is locked
                    ...
                    print(
                        f"Warning: File lock check failed for {file_path}: {response.status_code}",
                        file=sys.stderr,
                    )

            except Exception as e:
                print(
                    f"Warning: Could not check file lock for {file_path}: {e}",
                    file=sys.stderr,
                )
