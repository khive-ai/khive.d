...
class TestHeuristicAssessment:
    ...
    def test_heuristic_pattern_recognition(self, complexity_indicators):
        ...
        for task_description, expected_level in test_cases:
            ...
            assert isinstance(
                result, list
            ), f"Expected list result for '{task_description}'"
            ...
            assert (
                found_indicators
            ), f"No {expected_level} indicators found in '{task_description}'"
    ...
    def test_edge_case_handling(self):
        ...
        for edge_case in edge_cases:
            try:
                ...
                assert isinstance(
                    result, list
                ), f"Expected list for edge case: '{edge_case}'"
...
class TestTriageAlgorithms:
    ...
    async def test_triage_classification_accuracy(self, mock_triage, triage_scenarios):
        ...
        for scenario in triage_scenarios:
            ...
            mock_consensus = TriageConsensus(
                should_escalate=scenario["expected_tier"]
                in ["complex", "very_complex"],
                complexity_votes=(
                    {"simple": 1, "complex": 2}
                    if scenario["expected_tier"] in ["complex", "very_complex"]
                    else {"simple": 2, "complex": 1}
                ),
            ...
            expected_escalation = scenario["expected_tier"] in [
                "complex",
                "very_complex",
            ]
    ...
    async def test_agent_count_bounds(self, mock_triage):
        ...
        for tier, agent_count in test_cases:
            mock_consensus = TriageConsensus(
                should_escalate=tier
                in [ComplexityTier.COMPLEX, ComplexityTier.VERY_COMPLEX],
                complexity_votes=(
                    {"simple": 1, "complex": 2}
                    if tier in [ComplexityTier.COMPLEX, ComplexityTier.VERY_COMPLEX]
                    else {"simple": 2, "complex": 1}
                ),
                average_confidence=0.8,
            ...
            if result.final_agent_count is not None:
                assert (
                    1 <= result.final_agent_count <= 12
                ), f"Agent count {result.final_agent_count} outside bounds [1, 12]"
            expected_escalation = tier in [
                ComplexityTier.COMPLEX,
                ComplexityTier.VERY_COMPLEX,
            ]
            assert result.should_escalate == expected_escalation
...
class TestDecisionMatrix:
    ...
    def test_indicator_matching(self):
        ...
        for task, expected_tier in test_cases:
            ...
            assert (
                best_tier == expected_tier
            ), f"Expected {expected_tier}, got {best_tier} for '{task}'"
    ...
    def test_agent_count_ranges(self):
        ...
        for tier, range_str in ranges_by_tier.items():
            ...
            ), f"Invalid range for {tier}: {min_agents}-{max_agents}"
