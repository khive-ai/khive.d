...
"""
    ...
    def test_load_clean_config_with_cli_args(self, temp_dir):
        ...
        cli_args = Namespace(json_output=True, dry_run=True, verbose=True)
...
class TestBranchProtection:
    ...
    def test_is_branch_protected_case_sensitive(self, temp_dir):
        ...
        config = CleanConfig(
            project_root=temp_dir, protected_branch_patterns=["Release/*"]
        )
...
class TestGitOperations:
    ...
    @patch("khive.cli.khive_clean.git_run_clean")
    def test_detect_default_branch_clean_gh_success(
        self, mock_git_run, mock_cli_run, mock_which, temp_dir
    ):
    ...
    @patch("khive.cli.khive_clean.shutil.which")
    @patch("khive.cli.khive_clean.git_run_clean")
    def test_detect_default_branch_clean_symbolic_ref(
        self, mock_git_run, mock_which, temp_dir
    ):
    ...
    @patch("khive.cli.khive_clean.shutil.which")
    @patch("khive.cli.khive_clean.git_run_clean")
    def test_detect_default_branch_clean_fallback(
        self, mock_git_run, mock_which, temp_dir
    ):
...
class TestMainCleanFlow:
    ...
    def test_main_clean_flow_single_branch(
        self,
        mock_clean_branch,
        mock_git_run,
        mock_current_branch,
        ...
        config = CleanConfig(project_root=temp_dir)
        args = Namespace(all_merged=False, branch_name="feature/test", yes=False)
    ...
    def test_main_clean_flow_all_merged(
        self,
        mock_merged_branches,
        mock_git_run,
        mock_current_branch,
        mock_default_branch,
        mock_chdir,
        ...
        config = CleanConfig(project_root=temp_dir)
        args = Namespace(all_merged=True, branch_name=None, into=None, yes=True)
        ...
        mock_merged_branches.return_value = [
            "feature/merged1",
            "feature/merged2",
            "main",
        ]
    ...
    @patch("khive.cli.khive_clean.get_current_git_branch_clean")
    @patch("khive.cli.khive_clean.git_run_clean")
    def test_main_clean_flow_checkout_failure(
        self,
        mock_git_run,
        mock_current_branch,
        mock_default_branch,
...
class TestCLIIntegration:
    ...
    @patch("khive.cli.khive_clean._main_clean_flow")
    @patch(
        "khive.cli.khive_clean.sys.argv",
        ["khive-clean", "--json-output", "feature/test"],
    )
    ...
    @patch("khive.cli.khive_clean._main_clean_flow")
    @patch(
        "khive.cli.khive_clean.sys.argv", ["khive-clean", "--dry-run", "feature/test"]
    )
...
class TestErrorScenarios:
    ...
    @patch(
        "khive.cli.khive_clean.sys.argv",
        ["khive-clean", "--project-root", "/nonexistent", "feature/test"],
    )
