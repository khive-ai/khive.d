...
class TestCLIDispatcher:
    ...
    def test_load_command_module_unknown_command(
        self, mock_print, mock_help, mock_import
    ):
    ...
    def test_commands_exist_and_have_entry_points(self):
        ...
        for cmd_name, module_name in COMMANDS.items():
            ...
            try:
                ...
                assert hasattr(module, ENTRY_POINT_FUNCTION_NAME), (
                    f"Module {full_path} missing {ENTRY_POINT_FUNCTION_NAME}"
                )
                entry_point = getattr(module, ENTRY_POINT_FUNCTION_NAME)
                assert callable(entry_point), (
                    f"Entry point {ENTRY_POINT_FUNCTION_NAME} in {full_path} is not callable"
                )
