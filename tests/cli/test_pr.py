...
class TestPRConfig:
    ...
    def test_load_config_with_valid_toml(
        self, mock_read_text, mock_exists, mock_tomllib
    ):
    ...
    def test_load_config_with_invalid_toml(
        self, mock_warn, mock_read_text, mock_exists, mock_tomllib
    ):
...
class TestMainWorkflowErrors:
    ...
    def test_same_branch_error(
        self, mock_get_base, mock_get_branch, mock_which, mock_chdir
    ):
    ...
    def test_existing_pr_found(
        self, mock_existing_pr, mock_get_base, mock_get_branch, mock_which, mock_chdir
    ):
...
class TestDryRunMode:
    ...
    def test_dry_run_mode_basic(self):
        ...
        config = PRConfig(
            project_root=project_root, json_output=False, dry_run=True, verbose=False
        )
        ...
        with (
            ...
            patch(
                "khive.cli.khive_pr.get_current_branch_pr", return_value="feature/test"
            ),
...
class TestPRWorkflowIntegration:
    ...
    def test_push_failure_scenario(
        self,
        mock_git,
        mock_existing_pr,
        mock_get_base,
        ...
        project_root = Path("/tmp/test-repo")
        config = PRConfig(
            project_root=project_root, json_output=False, dry_run=False, verbose=False
        )
    ...
    def test_no_push_flag_handling(self):
        ...
        with (
            ...
            patch(
                "khive.cli.khive_pr.gh_run_pr", return_value=Mock(returncode=1)
            ),  # Fail PR creation
        ):
            args = Mock()
            args.base = None
