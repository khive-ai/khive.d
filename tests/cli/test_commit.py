...
class TestCommitConfig:
    ...
    def test_conventional_commit_regex_custom(self, tmp_path):
        ...
        config = CommitConfig(
            ...
            conventional_commit_regex_pattern=r"^(feat|fix): .+"
        )
        regex = config.conventional_commit_regex
...
class TestStageChanges:
    ...
    def test_stage_changes_all_mode_with_changes(self, tmp_path):
        ...
        with patch('khive.cli.khive_commit.git_run') as mock_git:
            ...
            add_success = subprocess.CompletedProcess(["git", "add", "-A"], returncode=0, stdout="", stderr="")
    ...
    def test_stage_changes_patch_mode_interactive(self, tmp_path):
        ...
        with patch('khive.cli.khive_commit.git_run') as mock_git:
            with patch('subprocess.run') as mock_subprocess:
                ...
                mock_subprocess.assert_called_once_with(["git", "add", "-p"], cwd=config.project_root)
...
class TestMainCommitFlow:
        ...
        with patch('khive.cli.khive_commit.shutil.which', return_value="/usr/bin/git"):
            with patch('os.chdir'):
                with patch('khive.cli.khive_commit.ensure_git_identity'):
                    with patch('khive.cli.khive_commit.stage_changes', return_value=True):
                        with patch('khive.cli.khive_commit.get_current_branch', return_value="main"):
                            with patch('khive.cli.khive_commit.git_run') as mock_git:
    ...
    def test_main_flow_nothing_to_commit(self, tmp_path):
        ...
        with patch('khive.cli.khive_commit.shutil.which', return_value="/usr/bin/git"):
            with patch('os.chdir'):
                with patch('khive.cli.khive_commit.ensure_git_identity'):
                    with patch('khive.cli.khive_commit.stage_changes', return_value=False):
    ...
    def test_main_flow_allow_empty_commit(self, tmp_path):
        ...
        with patch('khive.cli.khive_commit.shutil.which', return_value="/usr/bin/git"):
            with patch('os.chdir'):
                with patch('khive.cli.khive_commit.ensure_git_identity'):
                    with patch('khive.cli.khive_commit.stage_changes', return_value=False):  # No staging needed for empty
                        with patch('khive.cli.khive_commit.get_current_branch', return_value="main"):
                            with patch('khive.cli.khive_commit.git_run') as mock_git:
                                mock_git.side_effect = [
    ...
    def test_main_flow_commit_command_failed(self, tmp_path):
        ...
        with patch('khive.cli.khive_commit.shutil.which', return_value="/usr/bin/git"):
            with patch('os.chdir'):
                with patch('khive.cli.khive_commit.ensure_git_identity'):
                    with patch('khive.cli.khive_commit.stage_changes', return_value=True):
                        with patch('khive.cli.khive_commit.git_run', return_value=mock_process_fail):
    ...
    def test_main_flow_push_failed(self, tmp_path):
        ...
        with patch('khive.cli.khive_commit.shutil.which', return_value="/usr/bin/git"):
            with patch('os.chdir'):
                with patch('khive.cli.khive_commit.ensure_git_identity'):
                    with patch('khive.cli.khive_commit.stage_changes', return_value=True):
                        with patch('khive.cli.khive_commit.get_current_branch', return_value="feature/test"):
                            with patch('khive.cli.khive_commit.git_run') as mock_git:
...
class TestErrorHandlingEdgeCases:
    ...
    def test_project_root_detection_fallback(self):
        ...
        with patch('subprocess.check_output', side_effect=subprocess.CalledProcessError(1, "git")):
            ...
            assert khive.cli.khive_commit.PROJECT_ROOT == Path.cwd()
...
class TestCLIArgumentParsing:
        ...
        with patch('khive.cli.khive_commit._main_commit_flow') as mock_flow:
            ...
            with patch('khive.cli.khive_commit.warn_msg') as mock_warn:
                with patch('sys.argv', [
                    ...
                    '--type', 'fix',       # structured (should be ignored)
                ]):
                    with patch('khive.cli.khive_commit.PROJECT_ROOT', tmp_path):
                        main()
    ...
    def test_dry_run_mode_integration(self, tmp_path):
        ...
        with patch('khive.cli.khive_commit._main_commit_flow') as mock_flow:
            mock_flow.return_value = {"status": "success", "message": "Success (dry run)"}
