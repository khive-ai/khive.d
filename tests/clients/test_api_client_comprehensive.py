...
from khive.clients.api_client import APIClient
from khive.clients.errors import APIClientError, RateLimitError, ConnectionError as APIConnectionError
...
class TestAPIClient:
    ...
    async def test_connection_error_handling(self):
        ...
        with patch.object(client, '_make_request', side_effect=aiohttp.ClientConnectorError("Connection failed")):
    ...
    async def test_timeout_error_handling(self):
        ...
        with patch.object(client, '_make_request', side_effect=aiohttp.ServerTimeoutError("Request timeout")):
            with pytest.raises(APIClientError) as exc_info:
    ...
    async def test_json_decode_error_handling(self):
        ...
        mock_response = Mock(spec=ClientResponse)
        mock_response.status = 200
        mock_response.json = AsyncMock(side_effect=aiohttp.ContentTypeError("Invalid JSON"))
